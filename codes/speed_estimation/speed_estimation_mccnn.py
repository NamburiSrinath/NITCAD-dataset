...
for img_idx in range(61):
	...
	disp_t = cv2.imread(os.path.join(disp_dir,'%d.png'%(img_idx+1)))      # if using mccnn
	disp_tm1 = cv2.imread(os.path.join(disp_dir,'%d.png'%(img_idx+2)))
	...
	for obj_idx_t in range(count_t):
		...
		if int(tracked[9])-int(tracked[7]) > 90 and int(tracked[8])-int(tracked[6]) > 90:	
			...
			disp = disp_t[int(centroid_t[1]),int(centroid_t[0])][0]
			disp_tp1 = disp_tm1[int(centroid_tp1[1]),int(centroid_tp1[0])][0]
			...
			squared_error = abs(inst_depth - inst_depth_cld)
			print squared_error
			...
			cv2.putText(draw, '%.2f'%depth_t+' m, '+'error = '+'%.2f m'%squared_error, (int(tracked[6]), int(tracked[7])), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,0), 2)

			imageconcat = np.concatenate((draw, disp_t), axis=0)
			cv2.imwrite("image_"+"%04d.jpg"%(img_idx-1),imageconcat)
			
			cv2.imshow("output display", imageconcat)
			# print img_idx
