...
print "======= Monocular Visual Odometry on KITTI Tracking Dataset ======="

seq_id = 1
root_dir        = '/home/umamaheswaran/major_project/datasets/kitti_tracking/data_tracking_image_2/training/'
calib_dir       =  os.path.join(root_dir, 'calib')
imgdir   = os.path.join(root_dir,'image_02/%04d'%seq_id)
...
img1_col  = cv2.imread(os.path.join(imgdir,'000000.png'))
img1      = cv2.cvtColor(img1_col, cv2.COLOR_BGR2GRAY)

img2_col  = cv2.imread(os.path.join(imgdir,'000001.png'))
...
traj = np.zeros((750,600,3), np.uint8)
maskimg = np.zeros_like(img1_col)

cv2.namedWindow('Output Window',cv2.WINDOW_NORMAL)
cv2.resizeWindow('Output Window', 1842,750)
...
for idx in range(2,nimages):
	...
	currimg_color = cv2.imread(os.path.join(imgdir,'%06d.png'%idx))
	...
	imagec1 = np.concatenate((currimg_color,imgadd), axis=0)
	imagec2 = np.concatenate((imagec1,traj), axis=1)

	# cv2.rectangle(imagec2, (1292, 10), (1792, 60), (0,0,0), cv2.FILLED)
	text = " -------TRAJECTORY--------"
	cv2.putText(imagec2, text, (1400,50), cv2.FONT_HERSHEY_PLAIN, 1, (0,255,255), 1, 8)
	cv2.imshow('Output Window',imagec2)
	# cv2.imwrite("image_"+"%03d.jpg"%(idx-1),imagec2)
	cv2.waitKey(50)	
